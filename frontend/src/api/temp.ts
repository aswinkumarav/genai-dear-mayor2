...
export async function conversationApi(options: ConversationRequest, abortSignal: AbortSignal): Promise<Response> {
    const response = await fetch("/conversation", {
        ...
        body: JSON.stringify({
            ...
            containerName: "genai_innovationcenter"
        }),
        signal: abortSignal
    });
...
export const historyList = async (offset=0): Promise<Conversation[] | null> => {
    const response = await fetch(`/history/list?offset=${offset}&containerName=genai_innovationcenter`, {
        method: "GET",
    }).then(async (res) => {
...
export const historyEnsure = async (): Promise<CosmosDBHealth> => {
    const response = await fetch("/history/ensure?containerName=genai_innovationcenter", {
        method: "GET",
    })
    .then(async res => {
